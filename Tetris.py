...
class Tetris:
    ...
    def draw_text_middle(self,surface,text, size, color):  
        ...
        surface.blit(label, (top_left_x + play_width/2 - (label.get_width()/2) + 130, top_left_y + play_height/2 - label.get_height()/2 - 10))
    ...
    def draw_window(self,surface,grid,score=0,high_score=0):
        surface.fill((0,0,0))
        for i in range(len(grid)):
            for j in range(len(grid[i])):
        ...
        sx = top_left_x + play_width + 50
        sy = top_left_y + 10
        surface.blit(label, (sx - 35, sy + 280))
    ...
    def main(self,window):
        ...
        while run:
            ...
            if self.check_lost(locked_positions):
                self.draw_text_middle(window, "YOU LOST!",80,(255,255,255))
                pygame.display.update()
    ...
    def main_menu(self):
        ...
        while run:
            window.fill((0,0,0))
            self.draw_text_middle(window,"Press Any Key To Start",40,(255,255,255))
            pygame.display.update()
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    run = False
